...
export class TeamsService {
  ...
  public async getTeams(): Promise<CITeam[]> {
    return await this.http.get<TeamsResponse>(this.baseUri + "/").pipe(
      map((r: TeamsResponse) => {
  ...
  public async getTeam(team: string): Promise<CITeam> {
    return await this.http.get<TeamResponse>(this.baseUri + "/" + team).pipe(
      map((r: TeamResponse) => {
  ...
  public async getTeamMembers(team: string): Promise<CIUser[]> {
    return await this.http.get<UsersResponse>(this.baseUri + "/" + team + "/members").pipe(
      map((r: UsersResponse) => {
  ...
  public async getUsersByName(name: string): Promise<CIUser[]> {
    return await this.http.get<UsersResponse>("/api/users?name=" + name).pipe(
      map((r: UsersResponse) => {
        return r.users.map(u => Deserialize(u, CIUser));
      })
    ).toPromise();
